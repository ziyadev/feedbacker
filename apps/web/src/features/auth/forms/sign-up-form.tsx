...
export const EmailSection = () => {
  ...
  const errorMessages: { [key: string]: string } = {
    [CredentialsSignUpErrorCode.UserExists]:
  ...
  const [signup, { loading }] = useMutation(CREDENTIALS_SIGN_UP, {
    onCompleted: ({ credentialsSignUp: { user, errors } }) => {
      if (!errors) {
      ...
      for (const error of errors) {
        // if the error code is UserExists, set the error message on the email field
        if (error.code === CredentialsSignUpErrorCode.UserExists) {
          form.setFieldMeta('email', (prev) => ({
            ...prev,
            errorMap: {
              onChange: errorMessages[error.code],
            },
          }));
        }
      }
